...
class exporter(object):
    ...
    def export_boms(self):
        ...
        for i in bom_recs.read(bom_fields):
            ...
            if (
                ...
                # routing step.
                #
                yield '<operation name=%s size_multiple="1" posttime="P%dD"%s%s xsi:type="operation_routing">' "<item name=%s/><location name=%s/>\n" % (
                    quoteattr(operation),
                    self.manufacturing_lead,
    ...
    def export_manufacturingorders(self):
        ...
        recs = m.search([("state", "in", ["progress", "planned"])])
        fields = [
            "bom_id",
            "date_start",
            "date_planned_start",
